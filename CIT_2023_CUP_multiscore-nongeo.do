...
preserve
	...
	twoway (scatter running_sisben running_saber11 if tr ==0, ///
	       ...
	       (scatter running_sisben running_saber11 if tr ==1, ///
			...
			size(vlarge)) legend(off) graphregion(color(white)) ylabel(,angle(0) nogrid)
	graph export "outputs/Vol-2-LRS-MultiCutoff-LVRS-plot.png", replace
restore
...
**-------------------------------------**
sjlog using "outputs/Vol-2-STATA_LRS_rdms_threepoints", replace
	gen p1 = .
	gen p2 = .
	replace p1 = 0  in 1
	replace p2 = 0  in 1
...
**---------------------------------------------**
sjlog using "outputs/Vol-2-STATA_LRS_rdms_vs_rdrobust", replace
	global pdim1 = 30
	global pdim2 = 0
...
**--------------------------------------------------------------**
sjlog using "outputs/Vol-2-STATA_LRS_rdms_perpendicular_dist_step1", replace
...
**--------------------------------------------------------------**
sjlog using "outputs/Vol-2-STATA_LRS_rdms_perpendicular_dist_step2", replace
...
**-------------------------------------------**
sjlog using "outputs/Vol-2-STATA_LRS_rdms_perpendicular_dist_rdrobust", replace
...
**---------------------------------------**
sjlog using "outputs/Vol-2-STATA_LRS_rdms_perpendicular_dist", replace
